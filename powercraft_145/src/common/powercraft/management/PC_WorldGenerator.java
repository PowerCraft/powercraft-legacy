        ...
        for (Block block: Block.blocksList)
        {
            if (block instanceof PC_IWorldSpawnGenerate)
            {
                ...
                for (int i = 0; i < num; i++)
                {
                    ...
                    PC_VecI pos = wsg.getSpawnPoint(random);
                    pos.x += chunkX * 16;
                    pos.y = PC_MathHelper.clamp_int(pos.y, 1, 255);
                    pos.z += chunkZ * 16;
                    new PC_WorldGenMinableMetadata(block.blockID, wsg.getSpawnPointMetadata(random), blocksCount).generate(world, random, pos.x, pos.y, pos.z);
                }
            }
        }
    }
