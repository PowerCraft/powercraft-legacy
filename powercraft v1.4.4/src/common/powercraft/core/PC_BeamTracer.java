...
public class PC_BeamTracer {
	...
	public void forkBeam(PC_CoordI par_cnt, PC_CoordI par_move, PC_Color par_color, float strength, int limit) {
		...
		for (int c = 0; c < limit; c++) {
			...
			if (b!=null){
				res = result.FALLBACK;
				if(b instanceof PC_IBeamSpecialHandling){
					res = ((PC_IBeamSpecialHandling)b).onHitByBeamTracer(this, cnt, move, color, strength, limit-c);
				}
			}
	...
	public static void addLaser(World world, PC_CoordI cnt, PC_CoordI move, float strength, PC_Color color){
		cnt = cnt.copy();
		PC_CoordI oldMove = move;
		move = move.copy();
		color = color.copy();
