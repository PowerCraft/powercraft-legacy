...
public class PC_RedstoneUnisolatedTileEntity extends PC_CableTileEntity implements PC_IRedstoneCable {
	...
	protected static PC_RedstoneGrid getGridS(World world, int x, int y, int z, PC_Direction dir, int cableID){
		...
		if(te instanceof PC_TileEntityMultiblock){
			...
			if(multiblockTileEntity instanceof PC_RedstoneUnisolatedTileEntity){
				...
				if(cableID==-1)
					return ((PC_RedstoneIsolatedTileEntity)multiblockTileEntity).getGrid();
		...
		if(!isClient() && grid!=null){
			...
			for(PC_Direction dir2:PC_Direction.VALID_DIRECTIONS){
				...
				if(connection!=null){
					...
					if(connection.length>3 && (connection[3]&0xFFFF)!=0){
						PC_RedstoneGrid g = getGrid(dir.offsetX+dir2.offsetX, dir.offsetY+dir2.offsetY, dir.offsetZ+dir2.offsetZ, dir2.getOpposite());
						if(g!=null && !doneGrids.contains(g)){
							if(isFirst){
								isFirst = false;
							}else{
								PC_RedstoneGrid ng = new PC_RedstoneGrid();
								doneGrids.add(ng);
								setBlockAndNeighborGrid(dir.offsetX+dir2.offsetX, dir.offsetY+dir2.offsetY, dir.offsetZ+dir2.offsetZ, dir2.getOpposite(), ng);
							}
						}
					}
					if(connection.length>4 && (connection[4]&0xFFFF)!=0){
						PC_RedstoneGrid g = getGrid(dir.offsetX+dir2.offsetX, dir.offsetY+dir2.offsetY, dir.offsetZ+dir2.offsetZ, dir.getOpposite());
						if(g!=null && !doneGrids.contains(g)){
							if(isFirst){
								isFirst = false;
							}else{
								PC_RedstoneGrid ng = new PC_RedstoneGrid();
								doneGrids.add(ng);
								setBlockAndNeighborGrid(dir.offsetX+dir2.offsetX, dir.offsetY+dir2.offsetY, dir.offsetZ+dir2.offsetZ, dir.getOpposite(), ng);
							}
						}
					}
				}
	...
	private void setBlockAndNeighborGrid(int xOffset, int yOffset, int zOffset, PC_Direction dir, PC_RedstoneGrid grid) {
		setBlockAndNeighborGridS(multiblock.worldObj, multiblock.xCoord+xOffset, multiblock.yCoord+yOffset, multiblock.zCoord+zOffset, dir, -1, grid);
	}
	...
	protected static void setBlockAndNeighborGridS(World world, int x, int y, int z, PC_Direction dir, int cable, PC_RedstoneGrid grid) {
		...
		while(!toLookAt.isEmpty()){
			...
			if(tileEntity!=null){
				PC_MultiblockTileEntity multiblockTileEntity = tileEntity.getMultiblockTileEntity(PC_MultiblockIndex.FACEINDEXFORDIR[posAndRot.dir.ordinal()]);
				if(multiblockTileEntity instanceof PC_RedstoneUnisolatedTileEntity){
					...
					for(PC_Direction dir2:PC_Direction.VALID_DIRECTIONS){
						...
						if(connection!=null){
							...
							if(connection.length>1 && (connection[1]&0xFFFF)!=0){
								PC_RedstoneGrid g = getGridS(world, posAndRot.x + dir.offsetX, posAndRot.y + dir.offsetY, posAndRot.z + dir.offsetZ, dir2, -1);
								if(g!=null && g!=grid){
									PRC p = new PRC(posAndRot.x + dir.offsetX, posAndRot.y + dir.offsetY, posAndRot.z + dir.offsetZ, dir2, getCable(connection[1]&0xFFFF));
									if(!toLookAt.contains(p)){
										toLookAt.add(p);
									}
								}
							...
							if(connection.length>2 && (connection[2]&0xFFFF)!=0){
								PC_RedstoneGrid g = getGridS(world, posAndRot.x + dir2.offsetX, posAndRot.y + dir2.offsetY, posAndRot.z + dir2.offsetZ, dir, -1);
								if(g!=null && g!=grid){
									PRC p = new PRC(posAndRot.x + dir2.offsetX, posAndRot.y + dir2.offsetY, posAndRot.z + dir2.offsetZ, dir, getCable(connection[2]&0xFFFF));
									if(!toLookAt.contains(p)){
										toLookAt.add(p);
									}
								}
								g = getGridS(world, posAndRot.x + dir2.offsetX, posAndRot.y + dir2.offsetY, posAndRot.z + dir2.offsetZ, dir2.getOpposite(), -1);
								if(g!=null && g!=grid){
									PRC p = new PRC(posAndRot.x + dir2.offsetX, posAndRot.y + dir2.offsetY, posAndRot.z + dir2.offsetZ, dir2.getOpposite(), getCable(connection[2]&0xFFFF));
									if(!toLookAt.contains(p)){
										toLookAt.add(p);
									}
								}
							...
							if(connection.length>3 && (connection[3]&0xFFFF)!=0){
								PC_RedstoneGrid g = getGridS(world, posAndRot.x + dir.offsetX+dir2.offsetX, posAndRot.y + dir.offsetY+dir2.offsetY, posAndRot.z + dir.offsetZ+dir2.offsetZ, dir2.getOpposite(), -1);
								if(g!=null && g!=grid){
									PRC p = new PRC(posAndRot.x + dir.offsetX+dir2.offsetX, posAndRot.y + dir.offsetY+dir2.offsetY, posAndRot.z + dir.offsetZ+dir2.offsetZ, dir2.getOpposite(), getCable(connection[3]&0xFFFF));
									if(!toLookAt.contains(p)){
										toLookAt.add(p);
									}
								}
							...
							if(connection.length>4 && (connection[4]&0xFFFF)!=0){
								PC_RedstoneGrid g = getGridS(world, posAndRot.x + dir.offsetX+dir2.offsetX, posAndRot.y + dir.offsetY+dir2.offsetY, posAndRot.z + dir.offsetZ+dir2.offsetZ, dir.getOpposite(), -1);
								if(g!=null && g!=grid){
									PRC p = new PRC(posAndRot.x + dir.offsetX+dir2.offsetX, posAndRot.y + dir.offsetY+dir2.offsetY, posAndRot.z + dir.offsetZ+dir2.offsetZ, dir.getOpposite(), getCable(connection[4]&0xFFFF));
									if(!toLookAt.contains(p)){
										toLookAt.add(p);
									}
								}
					...
					for(PC_Direction dir2:PC_Direction.VALID_DIRECTIONS){
						...
						if(connection!=null){
							...
							if(connection.length>1 && (connection[1]&rCable.getCableType())!=0){
								PC_RedstoneGrid g = getGridS(world, posAndRot.x + dir.offsetX, posAndRot.y + dir.offsetY, posAndRot.z + dir.offsetZ, dir2, getCable(rCable.getCableType()));
								if(g!=null && g!=grid){
									PRC p = new PRC(posAndRot.x + dir.offsetX, posAndRot.y + dir.offsetY, posAndRot.z + dir.offsetZ, dir2, getCable(rCable.getCableType()));
									if(!toLookAt.contains(p)){
										toLookAt.add(p);
									}
								}
							...
							if(connection.length>2 && (connection[2]&rCable.getCableType())!=0){
								PC_RedstoneGrid g = getGridS(world, posAndRot.x + dir2.offsetX, posAndRot.y + dir2.offsetY, posAndRot.z + dir2.offsetZ, dir, getCable(rCable.getCableType()));
								if(g!=null && g!=grid){
									PRC p = new PRC(posAndRot.x + dir2.offsetX, posAndRot.y + dir2.offsetY, posAndRot.z + dir2.offsetZ, dir, getCable(rCable.getCableType()));
									if(!toLookAt.contains(p)){
										toLookAt.add(p);
									}
								}
								g = getGridS(world, posAndRot.x + dir2.offsetX, posAndRot.y + dir2.offsetY, posAndRot.z + dir2.offsetZ, dir2.getOpposite(), getCable(rCable.getCableType()));
								if(g!=null && g!=grid){
									PRC p = new PRC(posAndRot.x + dir2.offsetX, posAndRot.y + dir2.offsetY, posAndRot.z + dir2.offsetZ, dir2.getOpposite(), getCable(rCable.getCableType()));
									if(!toLookAt.contains(p)){
										toLookAt.add(p);
									}
								}
							...
							if(connection.length>3 && (connection[3]&rCable.getCableType())!=0){
								PC_RedstoneGrid g = getGridS(world, posAndRot.x + dir.offsetX+dir2.offsetX, posAndRot.y + dir.offsetY+dir2.offsetY, posAndRot.z + dir.offsetZ+dir2.offsetZ, dir2.getOpposite(), getCable(rCable.getCableType()));
								if(g!=null && g!=grid){
									PRC p = new PRC(posAndRot.x + dir.offsetX+dir2.offsetX, posAndRot.y + dir.offsetY+dir2.offsetY, posAndRot.z + dir.offsetZ+dir2.offsetZ, dir2.getOpposite(), getCable(rCable.getCableType()));
									if(!toLookAt.contains(p)){
										toLookAt.add(p);
									}
								}
							...
							if(connection.length>4 && (connection[4]&rCable.getCableType())!=0){
								PC_RedstoneGrid g = getGridS(world, posAndRot.x + dir.offsetX+dir2.offsetX, posAndRot.y + dir.offsetY+dir2.offsetY, posAndRot.z + dir.offsetZ+dir2.offsetZ, dir.getOpposite(), getCable(rCable.getCableType()));
								if(g!=null && g!=grid){
									PRC p = new PRC(posAndRot.x + dir.offsetX+dir2.offsetX, posAndRot.y + dir.offsetY+dir2.offsetY, posAndRot.z + dir.offsetZ+dir2.offsetZ, dir.getOpposite(), getCable(rCable.getCableType()));
									if(!toLookAt.contains(p)){
										toLookAt.add(p);
									}
								}
			...
			if(connection!=null){
				if(connection.length>1 && (connection[1]&0xFFFF)!=0){
					if(isIO(dir.offsetX, dir.offsetY, dir.offsetZ, dir2)){
						poweringBlocks.add(new PC_Vec3IWithRotation(multiblock.xCoord + dir.offsetX, multiblock.yCoord + dir.offsetY, multiblock.zCoord + dir.offsetZ, dir2));
					}
				}
				if(connection.length>2 && (connection[2]&0xFFFF)!=0){
					if(isIO(dir2.offsetX, dir2.offsetY, dir2.offsetZ, dir)){
						poweringBlocks.add(new PC_Vec3IWithRotation(multiblock.xCoord + dir2.offsetX, multiblock.yCoord + dir2.offsetY, multiblock.zCoord + dir2.offsetZ, dir));
					}
				}
				if(connection.length>3 && (connection[3]&0xFFFF)!=0){
					if(isIO(dir.offsetX+dir2.offsetX, dir.offsetY+dir2.offsetY, dir.offsetZ+dir2.offsetZ, dir2.getOpposite())){
						poweringBlocks.add(new PC_Vec3IWithRotation(multiblock.xCoord + dir.offsetX+dir2.offsetX, multiblock.yCoord + dir.offsetY+dir2.offsetY, multiblock.zCoord + dir.offsetZ+dir2.offsetZ, dir2.getOpposite()));
					}
				}
			}
			i++;
