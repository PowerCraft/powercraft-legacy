			...
			for(PC_Direction dir2:PC_Direction.VALID_DIRECTIONS){
				...
				if(connection!=null){
					...
					if(connection.length>3 && (connection[3]&cableType)!=0){
						useGrid(tileEntity.getGrid(dir.offsetX+dir2.offsetX, dir.offsetY+dir2.offsetY, dir.offsetZ+dir2.offsetZ, dir2.getOpposite(), cableType));
					}
					if(connection.length>4 && (connection[4]&cableType)!=0){
						useGrid(tileEntity.getGrid(dir.offsetX+dir2.offsetX, dir.offsetY+dir2.offsetY, dir.offsetZ+dir2.offsetZ, dir.getOpposite(), cableType));
					}
				}
				i++;
			}
		...
		if(!tileEntity.isClient() && grid!=null){
			...
			for(PC_Direction dir2:PC_Direction.VALID_DIRECTIONS){
				...
				if(connection!=null){
					...
					if(connection.length>3 && (connection[3]&cableType)!=0){
						PC_RedstoneGrid g = tileEntity.getGrid(dir.offsetX+dir2.offsetX, dir.offsetY+dir2.offsetY, dir.offsetZ+dir2.offsetZ, dir2.getOpposite(), cableType);
						if(g!=null && !doneGrids.contains(g)){
							if(isFirst){
								isFirst = false;
							}else{
								PC_RedstoneGrid ng = new PC_RedstoneGrid();
								doneGrids.add(ng);
								tileEntity.setBlockAndNeighborGrid(dir.offsetX+dir2.offsetX, dir.offsetY+dir2.offsetY, dir.offsetZ+dir2.offsetZ, dir2.getOpposite(), cableType, ng);
							}
						}
					}
					if(connection.length>4 && (connection[4]&cableType)!=0){
						PC_RedstoneGrid g = tileEntity.getGrid(dir.offsetX+dir2.offsetX, dir.offsetY+dir2.offsetY, dir.offsetZ+dir2.offsetZ, dir.getOpposite(), cableType);
						if(g!=null && !doneGrids.contains(g)){
							if(isFirst){
								isFirst = false;
							}else{
								PC_RedstoneGrid ng = new PC_RedstoneGrid();
								doneGrids.add(ng);
								tileEntity.setBlockAndNeighborGrid(dir.offsetX+dir2.offsetX, dir.offsetY+dir2.offsetY, dir.offsetZ+dir2.offsetZ, dir.getOpposite(), cableType, ng);
							}
						}
					}
				}
