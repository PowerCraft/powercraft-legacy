...
public abstract class PC_GresComponent{
	...
	public void putInRect(int x, int y, int width, int height) {
		if(width>maxSize.x && maxSize.x>=0)
			width = maxSize.x;
		if(height>maxSize.y && maxSize.y>=0)
			height = maxSize.y;
	...
	protected static PC_RectI setDrawRect(PC_RectI old, PC_RectI _new, double scale, int displayHeight){
		...
		GL11.glScissor((int)(rect.x * scale), displayHeight - (int)((rect.y + rect.height) * scale), (int)(rect.width * scale), (int)(rect.height * scale));
		return rect;
	}
	...
	protected void drawGradientRect(int x, int y, int width, int height, int colorTop, int colorBottom){
        float topAlpha = (float)(colorTop >> 24 & 255) / 255.0F;
        float topRed = (float)(colorTop >> 16 & 255) / 255.0F;
        float topGreen = (float)(colorTop >> 8 & 255) / 255.0F;
        float topBlue = (float)(colorTop & 255) / 255.0F;
        float bottomAlpha = (float)(colorBottom >> 24 & 255) / 255.0F;
        float bottomRed = (float)(colorBottom >> 16 & 255) / 255.0F;
        float bottomGreen = (float)(colorBottom >> 8 & 255) / 255.0F;
        float bottomBlue = (float)(colorBottom & 255) / 255.0F;
