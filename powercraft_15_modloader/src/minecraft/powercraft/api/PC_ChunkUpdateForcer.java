...
public class PC_ChunkUpdateForcer implements PC_IDataHandler, PC_ITickHandler {
	...
	private HashMap<PC_VecI, Integer> loadList(NBTTagCompound nbtTag,
			HashMap<PC_VecI, Integer> distance) {
	...
	public void load(NBTTagCompound nbtTag) {
		...
		for (int i = 0; i < count; i++) {
			...
			HashMap<PC_VecI, Integer> distance = loadList(
					nbtTag.getCompoundTag("value[" + i + "]"),
					new HashMap<PC_VecI, Integer>());
			chunks.put(dim, distance);
		}
	}

	private NBTTagCompound saveList(NBTTagCompound nbtTag,
			HashMap<PC_VecI, Integer> distance) {
	...
	public NBTTagCompound save(NBTTagCompound nbtTag) {
		...
		for (Entry<Integer, HashMap<PC_VecI, Integer>> e : chunks.entrySet()) {
			...
			nbtTag.setCompoundTag("value[" + i + "]",
					saveList(new NBTTagCompound(), e.getValue()));
			i++;
		}
		return nbtTag;
	}
