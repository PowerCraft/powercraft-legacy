...
public class PC_TreeHarvesting implements PC_ISpecialHarvesting {
			...
			for (int nx = x - 1; nx <= x + 1; nx++) {
				for (int ny = y - 1; ny <= y + 1; ny++) {
					for (int nz = z - 1; nz <= z + 1; nz++) {
						...
						if(block!=null){
							if(tree.woods.contains(new TreeState(block.blockID, meta))){
			...
			for (int nx = x - 1; nx <= x + 1; nx++) {
				for (int ny = y - 1; ny <= y + 1; ny++) {
					for (int nz = z - 1; nz <= z + 1; nz++) {
						...
						meta = PC_Utils.getMD(world, nx, ny, nz);
						if(block!=null){
							if(tree.leaves.contains(new TreeState(block.blockID, meta))){
			...
			if(block!=null){
				...
				while(i.hasNext()){
					...
					for(TreeState sampling:tree.saplings){
						if(sampling.blockID == drop.b.itemID && (sampling.metadata == drop.b.getItemDamage() || sampling.metadata==-1)){
	...
	public static void loadTrees() {
		...
		for(int i=0; i<4; i++){
			Tree tree = new Tree();
			tree.woods.add(new TreeState(Block.wood.blockID, i, 3));
			tree.leaves.add(new TreeState(Block.leaves.blockID, i, 3));
			tree.saplings.add(new TreeState(Block.sapling.blockID, i));
			trees.add(tree);
		}
	...
	private static class TreeState{
		...
		public TreeState(int blockID, int metadata){
			this.blockID = blockID;
			this.metadata = metadata;
			this.bitMask = -1;
		}
		
		public TreeState(int blockID, int metadata, int bitMask){
			this.blockID = blockID;
			this.metadata = metadata;
		...
		public boolean equals(Object obj) {
			...
			return other.blockID == blockID && ((other.metadata & bitMask) == (metadata & other.bitMask) || metadata==-1);
		}
