...
public class PC_GresScrollArea extends PC_GresWidget {
	...
	public PC_CoordI calcSize() {
		if(childs.size()>0){
			PC_CoordI cSize = childs.get(0).calcSize();
			if((type & VSCROLL)==0)
				size.y = cSize.y + 18;
			if((type & HSCROLL)==0)
				size.x = cSize.x + 18;
	...
	protected PC_RectI render(PC_CoordI offsetPos, PC_RectI scissorOld,
		...
		if(hScroll)
			renderTextureSliced(offsetPos.offset((int) hScrollPos + 1, size.y - 11), imgdir + "gres/scrollbar_handle.png", new PC_CoordI(
					hScrollSize - 1, 10), new PC_CoordI(0, 0), new PC_CoordI(256, 256));

		if(vScroll)
			renderTextureSliced(offsetPos.offset(size.x - 11, 1 + (int) vScrollPos), imgdir + "gres/scrollbar_handle.png", new PC_CoordI(
