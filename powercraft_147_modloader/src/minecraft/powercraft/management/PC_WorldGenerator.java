    ...
    public void generate(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator, IChunkProvider chunkProvider)
    {
        ...
        for (Block block: Block.blocksList)
        {
            if (block instanceof PC_IMSG)
            {
            	...
            	Object o = wsg.msg(PC_MSGRegistry.MSG_SPAWNS_IN_CHUNK, random);
            	if(o instanceof Integer){
	                int num = (Integer)o;
	
	                for (int i = 0; i < num; i++)
	                {
	                	o = wsg.msg(PC_MSGRegistry.MSG_BLOCKS_ON_SPAWN_POINT, random);
	                	int blocksCount = 2;
	                	if(o instanceof Integer)
	                		blocksCount = (Integer)o;
	                	o = wsg.msg(PC_MSGRegistry.MSG_SPAWN_POINT, random);
	                	if(o instanceof PC_VecI){
		                    ...
		                    pos.z += chunkZ * 16;
		                    int metadata = 0;
		                    o = wsg.msg(PC_MSGRegistry.MSG_SPAWN_POINT_METADATA, random);
		                    if(o instanceof Integer)
		                    	metadata = (Integer)o;
