...
public class PCtr_BeltHelper
{
    ...
    public static void doSpecialItemAction(World world, PC_VecI beltPos, EntityItem entity)
    {
        ...
        boolean flag = false;
        flag |= entity.func_92014_d().itemID == Item.bucketWater.shiftedIndex;
        flag |= entity.func_92014_d().itemID == Item.bucketEmpty.shiftedIndex;
        flag |= entity.func_92014_d().itemID == Item.glassBottle.shiftedIndex;
    ...
    {
        if (entity.func_92014_d().itemID == Item.bucketWater.shiftedIndex)
        {
            if (GameInfo.getBID(world, pos) == Block.cauldron.blockID && GameInfo.getMD(world, pos) < 3)
            {
            	ValueWriting.setMD(world, pos, 3);
                entity.func_92014_d().itemID = Item.bucketEmpty.shiftedIndex;
                return true;
            }
        }

        if (entity.func_92014_d().itemID == Item.bucketEmpty.shiftedIndex)
        {
            if (GameInfo.getBID(world, pos) == Block.waterStill.blockID || GameInfo.getBID(world, pos) == Block.waterMoving.blockID && GameInfo.getMD(world, pos) == 0)
            {
                ...
                entity.func_92014_d().itemID = Item.bucketWater.shiftedIndex;
                return true;
            }
        }

        if (entity.func_92014_d().itemID == Item.glassBottle.shiftedIndex)
        {
