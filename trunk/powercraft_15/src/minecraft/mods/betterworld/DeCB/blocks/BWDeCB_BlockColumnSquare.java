...
public class BWDeCB_BlockColumnSquare extends Block implements ITileEntityProvider {

	private int damVal;
	private Icon[] iconBuffer;
	private Class BlockColumnEntityClass;

	public BWDeCB_BlockColumnSquare(int par1) {
	...
	public TileEntity createNewTileEntity(World world) {
		return new BWDeCB_TileEntityBlockColumnSquare();
	}
	...
	public int getLightValue(IBlockAccess world, int x, int y, int z) {
		...
		if (block == null) {
			...
			return ((BWDeCB_TileEntityBlockColumnSquare) world.getBlockTileEntity(x, y,
					z)).getLightValue();
		}
	}
	...
	public Icon getBlockTexture(IBlockAccess par1iBlockAccess, int par2,
		...
		BWDeCB_TileEntityBlockColumnSquare tes = (BWDeCB_TileEntityBlockColumnSquare) par1iBlockAccess
	...
	public boolean removeBlockByPlayer(World world, EntityPlayer player, int x,
			int y, int z) {
		String userName = ((BWDeCB_TileEntityBlockColumnSquare) world
				.getBlockTileEntity(x, y, z)).getUserName();
		boolean lock = ((BWDeCB_TileEntityBlockColumnSquare) world.getBlockTileEntity(
				x, y, z)).isBlockLocked();
		...
		if (!world.isRemote
				&& !((EntityPlayerMP) player).theItemInWorldManager
						.isCreative()) {
			int damage = ((BWDeCB_TileEntityBlockColumnSquare) world
