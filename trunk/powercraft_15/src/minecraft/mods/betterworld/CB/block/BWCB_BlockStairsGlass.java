...
public class BWCB_BlockStairsGlass extends BlockStairs implements
		ITileEntityProvider {
	...
	public BWCB_BlockStairsGlass(int par1, Block par2Block, int par3) {
	...
	public TileEntity createNewTileEntity(World world) {
		return new BWCB_TileEntityBlockStairsGlass();
	}
	...
	public int getLightValue(IBlockAccess world, int x, int y, int z) {
		...
		if (block == null) {
			...
			return ((BWCB_TileEntityBlockStairsGlass) world.getBlockTileEntity(x, y,
					z)).getLightValue();
		}
	}
	...
	public Icon getBlockTexture(IBlockAccess par1iBlockAccess, int par2,
		...
		BWCB_TileEntityBlockStairsGlass tes = (BWCB_TileEntityBlockStairsGlass) par1iBlockAccess
	...
	public void registerIcons(IconRegister iconRegister) {
		iconBuffer = new Icon[BWCB_BlockList.blockGlassNormalTexture.size()];
		for (int i = 0; i < BWCB_BlockList.blockGlassNormalTexture.size(); i++) {
			iconBuffer[i] = iconRegister.registerIcon("betterworld/CB:"
					+ String.valueOf(BWCB_BlockList.blockGlassNormalTexture
	...
	public boolean removeBlockByPlayer(World world, EntityPlayer player, int x,
			int y, int z) {
		String userName = ((BWCB_TileEntityBlockStairsGlass) world
				.getBlockTileEntity(x, y, z)).getUserName();
		boolean lock = ((BWCB_TileEntityBlockStairsGlass) world.getBlockTileEntity(
				x, y, z)).isBlockLocked();
		...
		if (!world.isRemote
				&& !((EntityPlayerMP) player).theItemInWorldManager
						.isCreative()) {
			int damage = ((BWCB_TileEntityBlockStairsGlass) world
