...
public class BWCB_BlockStairsWoodR extends BlockStairs implements
		ITileEntityProvider {
	...
	public BWCB_BlockStairsWoodR(int par1, Block par2Block, int par3) {
		...
		setUnlocalizedName("BlockStairsWoodR");
	...
	public TileEntity createNewTileEntity(World world) {
		return new BWCB_TileEntityBlockStairsWood();
	}
	...
	public int getLightValue(IBlockAccess world, int x, int y, int z) {
		...
		if (block == null) {
			...
			return ((BWCB_TileEntityBlockStairsWood) world.getBlockTileEntity(x, y,
					z)).getLightValue();
		}
	}
	...
	public Icon getBlockTexture(IBlockAccess par1iBlockAccess, int par2,
		...
		BWCB_TileEntityBlockStairsWood tes = (BWCB_TileEntityBlockStairsWood) par1iBlockAccess
	...
	public void registerIcons(IconRegister iconRegister) {
		iconBuffer = new Icon[BWCB_BlockList.blockPlanksResistentTexture.size()];
		for (int i = 0; i < BWCB_BlockList.blockPlanksResistentTexture.size(); i++) {
			iconBuffer[i] = iconRegister.registerIcon("betterworld/CB:"
					+ String.valueOf(BWCB_BlockList.blockPlanksResistentTexture
	...
	public boolean removeBlockByPlayer(World world, EntityPlayer player, int x,
			int y, int z) {
		String userName = ((BWCB_TileEntityBlockStairsWood) world
				.getBlockTileEntity(x, y, z)).getUserName();
		boolean lock = ((BWCB_TileEntityBlockStairsWood) world.getBlockTileEntity(
				x, y, z)).isBlockLocked();
		...
		if (!world.isRemote
				&& !((EntityPlayerMP) player).theItemInWorldManager
						.isCreative()) {
			int damage = ((BWCB_TileEntityBlockStairsWood) world
