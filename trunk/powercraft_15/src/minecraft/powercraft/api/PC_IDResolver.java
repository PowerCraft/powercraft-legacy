...
public class PC_IDResolver {
	...
	public static NBTTagCompound makeIDTagCompound() {
		...
		TreeMap<String, PC_ItemArmor> itemArmors = PC_ItemRegistry
				.getPCItemArmors();
	...
	public static void savePCObjectsIDs(File worldDirectory) {
		...
		try {
			...
			CompressedStreamTools.writeCompressed(nbttag, new FileOutputStream(
					file));
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static boolean loadPCObjectsIDs(File worldDirectory) {
		...
		if (!file.exists())
			return false;
		try {
			loadIDFromTagCompound(
					CompressedStreamTools.readCompressed(new FileInputStream(
							file)), true);
		} catch (Exception e) {
			return false;
		}
	...
	public static int defaultID(PC_Block block) {
		...
		if (PC_ReflectHelper.getAnnotation(c, PC_Shining.class) != null) {
			List<Object> on = PC_ReflectHelper.getFieldsWithAnnotation(c,
					block, PC_Shining.ON.class);
			List<Object> off = PC_ReflectHelper.getFieldsWithAnnotation(c,
					block, PC_Shining.OFF.class);
	...
	public static int defaultID(PC_Item item) {
		...
		int id = module.getConfig().getInt(
				item.getClass().getSimpleName() + ".defaultID");
	...
	public static int defaultID(PC_ItemArmor itemArmor) {
		...
		int id = module.getConfig().getInt(
				itemArmor.getClass().getSimpleName() + ".defaultID");
		if (!PC_ItemRegistry.isItemIDFree(id))
			id = PC_ItemRegistry.getFreeItemID();
		return id;
	}
