...
public class PChg_HologramRenderBlocks extends PC_RenderBlocks {
	...
	public boolean shouldSideBeRendered(Block block, int x, int y, int z, int dir){
		if(block!=GameInfo.getBlock(blockAccess, x, y, z)){
			if (block instanceof PC_Block && dir == 1 && block.getRenderType() == PC_Renderer.getRendererID(true) && ((PC_Block)block).msg(PC_Utils.MSG_ROTATION, GameInfo.getMD(blockAccess, x, y, z))!=null)
				return false;
			return true;
		}
