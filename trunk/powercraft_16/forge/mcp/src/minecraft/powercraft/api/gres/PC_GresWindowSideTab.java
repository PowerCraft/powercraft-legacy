...
public class PC_GresWindowSideTab extends PC_GresContainer {
	...
	public static PC_GresComponent[] createRedstoneSideTab(){
		...
		PC_GresWindowSideTab sideTab = new PC_GresWindowSideTab("Redstone", new PC_GresDisplayObject(Item.redstone));
		...
		PC_GresDisplayObject dO = new PC_GresDisplayObject(
				...
				Block.torchRedstoneActive, Block.torchRedstoneIdle, 
	...
	public static PC_GresWindowSideTab createIOConfigurationSideTab(PC_TileEntityWithInventory te){
		...
		sideTab.add(frame);
		Object[] obj = new Object[te.getGroupCount()+1];
		...
		dO.setActiveDisplayObjectIndex(te.getSideGroup(0)+1);
		...
		dO.setActiveDisplayObjectIndex(te.getSideGroup(1)+1);
		sides[1].setLocation(new PC_Vec2I(1, 18));
		sides[1].setSize(new PC_Vec2I(16, 16));
		frame.add(sides[2] = new PC_GresDisplay(new PC_GresDisplayObject(te.getBlock().getIcon(PC_Direction.EAST, 0))));
		...
		dO.setActiveDisplayObjectIndex(te.getSideGroup(3)+1);
		...
		dO.setActiveDisplayObjectIndex(te.getSideGroup(4)+1);
		...
		dO.setActiveDisplayObjectIndex(te.getSideGroup(5)+1);
		sides[5].setLocation(new PC_Vec2I(35, 35));
		sides[5].setSize(new PC_Vec2I(16, 16));
		IOConfigEventListener eventListener = new IOConfigEventListener(te, sides);
	...
	private static class IOConfigEventListener implements PC_IGresEventListener{
		...
		private PC_TileEntityWithInventory tileEntity;
		private PC_GresDisplay[] sides;
		
		public IOConfigEventListener(PC_TileEntityWithInventory tileEntity, PC_GresDisplay[] sides){
				...
				if(bEvent.getEvent()==Event.CLICK){
					for(int i=0; i<sides.length; i++){
						if(sides[i] == bEvent.getComponent()){
							tileEntity.setSideGroup(i, sides[i].getDisplayObject().getActiveDisplayObjectIndex()-1);
							break;
						}
					}
				}
