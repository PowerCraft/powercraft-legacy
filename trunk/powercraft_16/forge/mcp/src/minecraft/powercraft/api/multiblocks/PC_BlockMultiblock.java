...
public class PC_BlockMultiblock extends PC_Block {
		...
		for(PC_MultiblockIndex index:PC_MultiblockIndex.values()){
			...
			if(collisionBoxes!=null){
				for(AxisAlignedBB collisionBox:collisionBoxes){
					setBlockBounds((float)collisionBox.minX, (float)collisionBox.minY, (float)collisionBox.minZ, 
							(float)collisionBox.maxX, (float)collisionBox.maxY, (float)collisionBox.maxZ);
					rayTrace = true;
	...
	public boolean removeBlockByPlayer(World world, EntityPlayer player, int x, int y, int z) {
		PC_TileEntityMultiblock tileEntity = PC_Utils.getTE(world, x, y, z);
		if(selectionIndex==null || world.isRemote)
			return false;
