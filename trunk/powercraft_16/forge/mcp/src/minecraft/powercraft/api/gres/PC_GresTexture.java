...
public class PC_GresTexture {
	...
	public void draw(int x, int y, int width, int height, int state) {
		...
		if(frame.y>0){
			...
			renderTextureSliced_static(x + frame.x, y, width - frame.x - frame.width, frame.y, 
					location.x + frame.x, location.y, size.x - frame.x - frame.width, size.y);
			if(frame.width>0){
				drawTexturedModalRect(x + width - frame.width, y, location.x + size.x - frame.width, location.y, frame.width, frame.y);
			}
		}
		if(frame.x>0){
			renderTextureSliced_static(x, y + frame.y,frame.x, height - frame.y - frame.height, 
					location.x, location.y + frame.y, size.x, size.y - frame.y - frame.height);
		}
		
		renderTextureSliced_static(x + frame.x, y + frame.y, width - frame.x - frame.width, height - frame.y - frame.height, 
				location.x + frame.x, location.y + frame.y, size.x - frame.x - frame.width, size.y - frame.y - frame.height);
		
		if(frame.width>0){
			renderTextureSliced_static(x + width - frame.width, y + frame.y, frame.width, height - frame.y - frame.height, 
					location.x + size.x - frame.width, location.y + frame.y, frame.width, size.y - frame.y - frame.height);
		}
		...
		if(frame.height>0){
			...
			renderTextureSliced_static(x + frame.x, y + height - frame.height, width - frame.x - frame.width, frame.height, 
					location.x + frame.x, location.y + size.y - frame.height, size.x - frame.x - frame.width, frame.height);
			if(frame.width>0){
				drawTexturedModalRect(x + width - frame.width, y + height - frame.height, location.x + size.x - frame.width, location.y + size.y - frame.height, frame.width, frame.height);
			}
		}
