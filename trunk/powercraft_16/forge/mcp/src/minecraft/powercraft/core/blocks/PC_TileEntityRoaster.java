				...
				for(EntityItem item:itemsList){
					if(energy>0.5){
						if(item.getEntityItem().stackSize>0){
							EntityItem eitem = new EntityItem(worldObj, item.posX - 0.1F + random.nextFloat() * 0.2F, item.posY, item.posZ
			                        - 0.1F + random.nextFloat() * 0.2F, PC_Utils.getSmeltingResult(item.getEntityItem()).splitStack(1));
