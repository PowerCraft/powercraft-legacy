...
import powercraft.apiOld.multiblocks.cable.redstone.PC_IRedstoneCable;
...
public abstract class PC_CableTileEntity extends PC_MultiblockTileEntity {
	...
	private static boolean[][] fix = {	{false,	false,	false,	false,	false,	false},
	...
	public void renderWorldBlock(RenderBlocks renderer) {
		...
		float s = m?4 / 16.0f:thickness / 16.0f; //TODO
		float w = m?8 / 32.0f:width / 32.0f; //TODO
	...
	public void renderCable(double w, int[] c) {
		...
		if(m){	
			...
			if(count>2) cDir=-2;
			else if(count==0) cDir=0;
			else if(count==1) cDir=(c[0]>0||c[1]>0)?0:1;
			else if((c[0]!=c[1] && (c[0]==0||c[1]==0)) || (c[2]!=c[3] && (c[2]==0||c[3]==0))){
				cDir = -2;
			}else if((c[0]>1&&c[1]>0)||(c[2]>0&&c[3]>0)){
				cDir = (c[0]>0&&c[1]>0)?0:1;
			}
		}else{
			if(c[0]==1 && c[1]==1){
				cDir=0;
		...
		if(useOverlay() && cDir!=-2){
			...
			for (int k = 0; k < 16; k++) {
				if ((getMask() & 1 << k) != 0) {
					Icon icon = getCableLineIcon(m?k:j);//TODO k-> j
					if (icon != null) {
						i = 0;
						for (PC_Direction dir2 : PC_Direction.VALID_DIRECTIONS) {
							if (dir2 == dir || dir2.getOpposite() == dir){
	...
	public void renderCable2(double w, double wl, double l, double min, double max, float s, int mask, boolean ending) {
		...
		if(useOverlay()){
			...
			for (int i = 0; i < 16; i++) {
				if ((mask & 1 << i) != 0) {
					Icon icon = getCableLineIcon(m?i:j);//TODO i-> j
					if (icon != null) {
