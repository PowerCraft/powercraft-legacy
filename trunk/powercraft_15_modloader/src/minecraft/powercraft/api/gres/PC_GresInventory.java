	...
	protected PC_RectI render(PC_VecI posOffset, PC_RectI scissorOld, double scale) {
		...
		for (int x = 0; x < gridWidth; x++) {
			for (int y = 0; y < gridHeight; y++) {
				renderTextureSlicedColored(posOffset.offset(x * slotWidth, y * slotHeight, 0), imgdir + "widgets.png", new PC_VecI(slotWidth, slotHeight),
						new PC_VecI(0, 66), new PC_VecI(18, 18), new PC_RectI(1, 1, 1, 1));
			}
		}
	...
	public List<String> getTooltip(PC_VecI mousePos) {
		...
		if (slot != null) {
			...
			if (itemstack != null) {
				...
				l.set(0, (new StringBuilder()).append("\247").append(Integer.toHexString(itemstack.getRarity().rarityColor)).append(l.get(0)).append("\2477")
						.toString());
				return l;
			}
		}
