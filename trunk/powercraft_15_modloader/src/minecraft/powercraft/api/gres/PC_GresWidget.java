...
public abstract class PC_GresWidget extends Gui implements PC_ITileEntityWatcher {
	 ...
	 *            height
	 * @param calcParent
	 *            flag whether to ask parent for position recalculation
	 * @return this
	 */
	 ...
	 * 
	 * @param removewidget
	 *            widget to remove from child list
	 * @return this
	 */
	 ...
	 * 
	 * @param text
	 *            text to draw (usually the label)
	 * @param x
	 *            pos x
	 * @param y
	 *            pos y
	 ...
	 * 
	 * @param offset
	 *            offset relative to root top left
	 * @param texture
	 *            texture to render (filename)
	 * @param rectSize
	 *            size of the rendered texture
	 * @param imgOffset
	 *            offset within the texture image (from top left)
	 */
	 ...
	 * 
	 * @param gui
	 *            the gui being drawed on
	 * @param texture
	 *            texture to render (filename)
	 * @param startPos
	 *            left top corner absolute position
	 * @param rectSize
	 *            size of the rendered texture
	 * @param imgOffset
	 *            offset within the texture image (from top left)
	 ...
	 *            offset within the texture image (from top left)
	 * @param imgSize
	 *            size of the whole "scalable" region in texture file (eg.
	...
	protected void renderTextureSlicedColored(PC_VecI offset, String texture, PC_VecI rectSize, PC_VecI imgOffset, PC_VecI imgSize, PC_RectI frame) {
		renderTextureSliced_static(this, offset.offset(pos), texture, rectSize, imgOffset, imgSize, frame, color[enabled ? (isMouseOver ? textColorHover
				: textColorEnabled) : textColorDisabled]);
	}
	 ...
	 *            the gui being drawed on
	 * @param startPos
	 *            offset relative to parent top left
	 * @param texture
	 *            texture to render (filename)
	 * @param rectSize
	 *            rectangle size
	...
	protected static void renderTextureSliced_static(Gui gui, PC_VecI startPos, String texture, PC_VecI rectSize, PC_VecI imgOffset, PC_VecI imgSize,
		...
		if (frame.y > 0) {
			...
			renderTextureSliced_static(gui, new PC_VecI(startPos.x + frame.x, startPos.y), new PC_VecI(rectSize.x - frame.x - frame.width, frame.y),
		...
		if (frame.x > 0) {
			renderTextureSliced_static(gui, new PC_VecI(startPos.x, startPos.y + frame.y), new PC_VecI(frame.x, rectSize.y - frame.y - frame.height),
					new PC_VecI(imgOffset.x, imgOffset.y + frame.y), new PC_VecI(imgSize.x, imgSize.y - frame.y - frame.height));
		}
		
		renderTextureSliced_static(gui, new PC_VecI(startPos.x + frame.x, startPos.y + frame.y), new PC_VecI(rectSize.x - frame.x - frame.width, rectSize.y
				- frame.y - frame.height), new PC_VecI(imgOffset.x + frame.x, imgOffset.y + frame.y), new PC_VecI(imgSize.x - frame.x - frame.width, imgSize.y
				- frame.y - frame.height));
		
		if (frame.width > 0) {
			renderTextureSliced_static(gui, new PC_VecI(startPos.x + rectSize.x - frame.width, startPos.y + frame.y), new PC_VecI(frame.width, rectSize.y
					- frame.y - frame.height), new PC_VecI(imgOffset.x + imgSize.x - frame.width, imgOffset.y + frame.y), new PC_VecI(frame.width, imgSize.y
					- frame.y - frame.height));
		}
		
		if (frame.height > 0) {
			if (frame.x > 0) {
				gui.drawTexturedModalRect(startPos.x, startPos.y + rectSize.y - frame.height, imgOffset.x, imgOffset.y + imgSize.y - frame.height, frame.x,
						frame.height);
			}
			renderTextureSliced_static(gui, new PC_VecI(startPos.x + frame.x, startPos.y + rectSize.y - frame.height), new PC_VecI(rectSize.x - frame.x
					- frame.width, frame.height), new PC_VecI(imgOffset.x + frame.x, imgOffset.y + imgSize.y - frame.height), new PC_VecI(imgSize.x - frame.x
					- frame.width, frame.height));
			if (frame.width > 0) {
				gui.drawTexturedModalRect(startPos.x + rectSize.x - frame.width, startPos.y + rectSize.y - frame.height, imgOffset.x + imgSize.x - frame.width,
						imgOffset.y + imgSize.y - frame.height, frame.width, frame.height);
			}
		}
	...
	protected static void drawButton(PC_GresWidget widget, PC_VecI pos, PC_VecI size, String text, int state) {
		...
		widget.drawCenteredString(widget.getFontRenderer(), text, pos.x + size.x / 2, pos.y + (size.y - widget.getFontRenderer().FONT_HEIGHT) / 2, txC);
	}
