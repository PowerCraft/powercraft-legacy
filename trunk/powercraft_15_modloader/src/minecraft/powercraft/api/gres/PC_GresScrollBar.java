...
public class PC_GresScrollBar extends PC_GresWidget {
	...
	protected PC_RectI render(PC_VecI posOffset, PC_RectI scissorOld, double scale) {
		...
		drawHorizontalLine(posOffset.x + pos.x, posOffset.x + pos.x + size.x - 1, posOffset.y + pos.y + size.y - 1, 0xffA0A0A0);
		drawVerticalLine(posOffset.x + pos.x, posOffset.y + pos.y, posOffset.y + pos.y + size.y - 1, 0xffA0A0A0);
		drawVerticalLine(posOffset.x + pos.x + size.x - 1, posOffset.y + pos.y, posOffset.y + pos.y + size.y - 1, 0xffA0A0A0);
		
		renderTextureSliced(posOffset.copy().add(size.x - 11, 1 + (int) scrollPos, 0), imgdir + "scrollbar_handle.png", new PC_VecI(
				10, scrollSize - 1), new PC_VecI(0, 0), new PC_VecI(256, 256), new PC_RectI(1, 1, 1, 1));
		return null;
	}
