...
public class PC_GresInventory extends PC_GresWidget {
	...
	PC_GresInventory(Slot slot[][]){
		super(slot.length*182, slot[0].length*18);
		canAddWidget = false;
		this.slot = slot;
	}
	...
	public PC_CoordI getMinSize() {
		return new PC_CoordI(slot.length*182, slot[0].length*18);
	}
	...
	protected void render(PC_CoordI posOffset) {
		...
		for(int x=0; x<slot.length; x++)
			for(int y=0; y<slot[x].length; y++){
				slot[x][y].xDisplayPosition = posOffset.x + pos.x + x*18 + 1;
				slot[x][y].yDisplayPosition = posOffset.y + pos.y + y*18 + 1;
