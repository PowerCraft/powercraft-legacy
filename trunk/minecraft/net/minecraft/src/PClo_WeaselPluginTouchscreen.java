 ...
 *
 */
public class PClo_WeaselPluginTouchscreen extends PClo_WeaselPlugin implements WeaselBitmapProvider {
	...
	public WeaselObject callProvidedFunction(WeaselEngine engine, String functionName, WeaselObject[] args) {
		...
		if(functionName.equals(getName()+".toDisk")) {
			...
			for (NetworkMember member : getNetwork().getMembers().values()) {
				if (member instanceof PClo_WeaselPluginDiskDrive) {
					disk = ((PClo_WeaselPluginDiskDrive) member).getImageDisk(Calc.toString(args[0]));					
					if (disk != null) break;
				}
			}
			if(disk == null) {
			...
			if(args.length == 1) {
				PClo_ItemWeaselDisk.setImageData(disk, screen);
				return null;
			}else {
		...
		if(functionName.equals(getName()+".fromDisk")) {
			...
			if(args.length == 1) {
				...
				PC_BitmapUtils.image(screen, PClo_ItemWeaselDisk.getImageData(disk), 0, 0);
				return null;
			}else {
				throw new ArrayIndexOutOfBoundsException();
			}
