...
public class PClo_WeaselPluginDiskDrive extends PClo_WeaselPlugin implements PC_IInventoryWrapper{
	...
	public WeaselObject getVariable(String name) {		
		...
		for(int i=0; i<disks.size(); i++) {	
			...
			if(WDT.getType(disk)==WDT.TEXT) {				
				if(name.equals(getName()+"."+i)) return new WeaselString(WDT.getText(disk));
				if(name.equals("text."+WDT.getLabel(disk))) return new WeaselString(WDT.getText(disk));
				continue;
			}
			
			if(WDT.getType(disk)==WDT.NUMBERLIST || WDT.getType(disk)==WDT.STRINGLIST) {			
				if(name.equals(getName()+"."+i+".size")) return new WeaselInteger(WDT.getListLength(disk));				
				if(name.equals(getName()+"."+i+".length")) return new WeaselInteger(WDT.getListLength(disk));		
	...
	public void setVariable(String name, Object object) {
		...
		for(int i=0; i<disks.size(); i++) {	
			...
			if(WDT.getType(disk)==WDT.TEXT) {				
				if(name.equals(getName()+"."+i) || name.equals("text."+WDT.getLabel(disk))) {
	...
	public List<String> getProvidedVariableNames() {
		...
		for(int i=0; i<disks.size(); i++) {	
			...
			if(WDT.getType(disk)==WDT.NUMBERLIST || WDT.getType(disk)==WDT.STRINGLIST) {				
				list.add(getName()+"."+i+".size");				
				list.add(getName()+"."+i+".length");
	...
	public ItemStack getImageDisk(String name) {
		for(int i=0; i<slots.getSizeInventory(); i++) {
			...
			if(getDiskName(i).equals(name) && getDiskType(i) == WDT.IMAGE) return getDisk(i);
		}
		return null;
	}
