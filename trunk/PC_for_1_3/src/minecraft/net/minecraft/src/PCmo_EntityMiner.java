...
//		motionZ = velocityZ;
	...
	public List<String> getLibraryFunctionNames(){
		...
		for (int i = 0; i < disks.size(); i++) {
			...
			if (PCnt_ItemWeaselDisk_UNUSED.getType(disk) == PCnt_ItemWeaselDisk_UNUSED.LIBRARY) {
				
				List<Instruction> ilist = PCnt_ItemWeaselDisk_UNUSED.getLibraryInstructions(disk);
	...
	public List<Instruction> getAllLibraryInstructions(){
		...
		for (int i = 0; i < disks.size(); i++) {
			...
			if (PCnt_ItemWeaselDisk_UNUSED.getType(disk) == PCnt_ItemWeaselDisk_UNUSED.LIBRARY) {
				ilist.addAll(PCnt_ItemWeaselDisk_UNUSED.getLibraryInstructions(disk));	
				continue;
			}
		}
		return ilist;
