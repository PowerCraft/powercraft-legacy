...
public class PCnt_WeaselPluginCore extends PCnt_WeaselPlugin implements IWeaselHardware {
	 ...
	 * CORE plugin
	 */
	public PCnt_WeaselPluginCore() {
		super();
		if(isMaster()){
			network = new PCnt_WeaselNetwork();
			network.registerMember(this);
			network.setName(Calc.generateUniqueName());
			network.setColor(PC_Color.pureRandomColor());
			PCnt_WeaselManager.registerNetwork(network);
		}
	}
	 ...
	 * CORE plugin
	 * 
	 * @param id weasel plugin id
	 */
	public PCnt_WeaselPluginCore(int id) {
		super(id);
		if(isMaster()){
			network = new PCnt_WeaselNetwork();
			network.registerMember(this);
...
//		}
	...
	public void restartAllNetworkDevices() {
		...
		for (PCnt_WeaselPlugin member : network.iterator()) {
			if (member != null && member != this)member.restartDevice();
		}
	...
	private WeaselObject fnBus(WeaselEngine engine, String functionName, WeaselObject[] args) throws BadFunc {
		...
		if (functionName.equals("nset")) {
			...
			PCnt_WeaselManager.setGlobalVariable(Calc.toString(args[0].get()), args[1]);
			return null;

		} else if (functionName.equals("nget")) {

			return PCnt_WeaselManager.getGlobalVariable(Calc.toString(args[0].get()));

		} else if (functionName.equals("nhas")) {

			return new WeaselBoolean(PCnt_WeaselManager.hasGlobalVariable(Calc.toString(args[0].get())));
			...
			return new WeaselBoolean(network.getMember(Calc.toString(args[0].get()))!=null);
			
		} else if (functionName.equals("hasDisk")) {
			boolean found = false;
			fl: for (PCnt_WeaselPlugin member : network.iterator()) {
				if (member != null && member != this && member instanceof PCnt_WeaselPluginDiskDrive) {
	...
	public void onDeviceDestroyed() {
		if(isMaster()) PCnt_WeaselManager.removeNetwork(network);
	}
