...
public class BWDeCB_BlockWallStone extends BlockWall implements ITileEntityProvider {

	private int damVal;
	private Icon[] iconBuffer;


	public BWDeCB_BlockWallStone(int par1, Block pBlock) {
		super(par1, pBlock);
		
		this.isBlockContainer = true;
		setUnlocalizedName("BlockWallStone");
	...
	public TileEntity createNewTileEntity(World world) {
		return new BWDeCB_TileEntityBlockWallStone();
	}
	...
	public int getLightValue(IBlockAccess world, int x, int y, int z) {
		...
		if (block == null) {
			...
			return ((BWDeCB_TileEntityBlockWallStone) world.getBlockTileEntity(x, y,
					z)).getLightValue();
		}
	}
	@Override
    public boolean canConnectWallTo(IBlockAccess par1IBlockAccess, int par2, int par3, int par4)
    {
        ...
        if (l != this.blockID && l != Block.fenceGate.blockID && l != BWCB.blockStone.blockID && l != BWDeCB.blockFenceWoodGate.blockID&& l != BWCB.blockStoneR.blockID)
        {
	...
	public Icon getBlockTexture(IBlockAccess par1iBlockAccess, int par2,
		...
		BWDeCB_TileEntityBlockWallStone tes = (BWDeCB_TileEntityBlockWallStone) par1iBlockAccess
	...
	public boolean removeBlockByPlayer(World world, EntityPlayer player, int x,
			int y, int z) {
		String userName = ((BWDeCB_TileEntityBlockWallStone) world
				.getBlockTileEntity(x, y, z)).getUserName();
		boolean lock = ((BWDeCB_TileEntityBlockWallStone) world.getBlockTileEntity(
				x, y, z)).isBlockLocked();
		...
		if (!world.isRemote
				&& !((EntityPlayerMP) player).theItemInWorldManager
						.isCreative()) {
			int damage = ((BWDeCB_TileEntityBlockWallStone) world
