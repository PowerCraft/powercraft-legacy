...
public class BWCB_BlockStairsStoneR extends BlockStairs implements
		ITileEntityProvider {
	...
	public BWCB_BlockStairsStoneR(int par1, Block par2Block, int par3) {
	...
	public TileEntity createNewTileEntity(World world) {
		return new BWCB_TileEntityBlockStairsStone();
	}
	...
	public int getLightValue(IBlockAccess world, int x, int y, int z) {
		...
		if (block == null) {
			...
			return ((BWCB_TileEntityBlockStairsStone) world.getBlockTileEntity(x, y,
					z)).getLightValue();
		}
	}
	...
	public Icon getBlockTexture(IBlockAccess par1iBlockAccess, int par2,
		...
		BWCB_TileEntityBlockStairsStone tes = (BWCB_TileEntityBlockStairsStone) par1iBlockAccess
	...
	public void registerIcons(IconRegister iconRegister) {
		iconBuffer = new Icon[BWCB_BlockList.blockStoneResistentTexture.size()];
		for (int i = 0; i < BWCB_BlockList.blockStoneResistentTexture.size(); i++) {
			iconBuffer[i] = iconRegister.registerIcon("betterworld/CB:"
					+ String.valueOf(BWCB_BlockList.blockStoneResistentTexture
	...
	public boolean removeBlockByPlayer(World world, EntityPlayer player, int x,
			int y, int z) {
		String userName = ((BWCB_TileEntityBlockStairsStone) world
				.getBlockTileEntity(x, y, z)).getUserName();
		boolean lock = ((BWCB_TileEntityBlockStairsStone) world.getBlockTileEntity(
				x, y, z)).isBlockLocked();
		...
		if (!world.isRemote
				&& !((EntityPlayerMP) player).theItemInWorldManager
						.isCreative()) {
			int damage = ((BWCB_TileEntityBlockStairsStone) world
