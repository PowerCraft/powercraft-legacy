...
public abstract class PCws_WeaselPlugin implements PC_INBT<PCws_WeaselPlugin>, PCws_IWeaselNetworkDevice {
	...
	private int[] weaselOutport = { 0, 0, 0, 0, 0, 0 };
	private int[] weaselInport = { 0, 0, 0, 0, 0, 0 };
	...
	public final PCws_WeaselPlugin readFromNBT(NBTTagCompound nbttag) {
		...
		for(int i=0; i<6; i++){
			weaselOutport[i] = nbttag.getInteger("weaselOutport["+i+"]");
			weaselInport[i] = nbttag.getInteger("weaselInport["+i+"]");
		}
		if(getNetwork()!=null)
	...
	public final NBTTagCompound writeToNBT(NBTTagCompound nbttag) {
		...
		for(int i=0; i<6; i++){
			nbttag.setInteger("weaselOutport["+i+"]", weaselOutport[i]);
			nbttag.setInteger("weaselInport["+i+"]", weaselInport[i]);
		}
		return nbttag;
	}
			...
			for(int i=0; i<6; i++){
				if(weaselInport[i] != newWeaselInport[i]){
					...
					if(weaselNetwork!=null){
						if(!weaselNetwork.callFunctionOnEngine("onPortChange."+name+"."+numToPort(i), new WeaselDouble(weaselInport[i])))
							anyNoCall = true;
					}else{
						if(doesProvideFunctionOnEngine("onPortChange."+name+"."+numToPort(i))){
							callFunctionOnEngine("onPortChange."+name+"."+numToPort(i), new WeaselDouble(weaselInport[i]));
						}else{
							anyNoCall = true;
						}
					}
