...
public class PC_GresTextEdit extends PC_GresWidget {
	...
	PC_GresTextEdit(String label, int chars){
	...
	PC_GresTextEdit(String label, int chars, int type){
		super((chars+1)*10, PC_Utils.mc().fontRenderer.FONT_HEIGHT + 12, label);
		maxChars = chars;
		canAddWidget = false;
		color[textColorEnabled] = 0xffffffff;
	...
	protected void render(int xOffset, int yOffset) {
		...
		if(mouseSelectStart!=mouseSelectEnd && hasFocus){
			...
			drawRect(xOffset + x + getStringLength(label.substring(0, s)) + 6, yOffset + y + 4, xOffset + x + getStringLength(label.substring(0, e)) + 6, yOffset + y + height - 5, 0xff3399FF);
			
		}
		
		drawString(label, xOffset + x + 6, yOffset + y + (height - 8) / 2);
		
		if(mouseSelectEnd==label.length()){
			if(hasFocus&& (cursorCounter / 6) % 2 == 0)
				drawString("_", xOffset + x + getStringLength(label) + 6, yOffset + y + (height - 8) / 2);
		}else
			if(hasFocus&& (cursorCounter / 6) % 2 == 0)
				drawVerticalLine(xOffset + x + getStringLength(label.substring(0, mouseSelectEnd)) + 5, yOffset + y + 3, yOffset + y + height - 5, color[enabled?textColorEnabled:textColorDisabled]);
	...
	public boolean mouseClick(int x, int y, int key) {
		...
		if(!enabled)
			return false;
		if(key!=-1){
			mouseSelectStart = getMousePositionInString(x);
			mouseSelectEnd = getMousePositionInString(x);
			mousePressed = true;
			return true;
		}
		return false;
				...
				switch(type){
					...
					case NUMBER:
						...
						if(Character.isDigit(Character.valueOf(c)))
							...
							if((mouseSelectStart==0||mouseSelectEnd==0)&&key == Keyboard.KEY_MINUS)
								addKey(c);
						break;
					default:
